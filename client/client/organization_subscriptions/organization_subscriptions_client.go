...
// New creates a new organization subscriptions API client with basic auth credentials.
...
// New creates a new organization subscriptions API client with a bearer token for authentication.
// It takes the following parameters:
// - host: http host (github.com).
// - basePath: any base path for the API client ("/v1", "/v3").
// - scheme: http scheme ("http", "https").
...
/*
CreateSubscription Creates a subscription for the Organization
*/
func (a *Client) CreateSubscription(params *CreateSubscriptionParams, authInfo runtime.ClientAuthInfoWriter, opts ...ClientOption) (*CreateSubscriptionNoContent, error) {
	...
	success, ok := result.(*CreateSubscriptionNoContent)
	if ok {
		return success, nil
	}
