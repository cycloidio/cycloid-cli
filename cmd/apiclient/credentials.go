...
func (m *apiClient) CreateCredential(org, name, credentialType string, rawCred *models.CredentialRaw, path, canonical, description string) (*models.Credential, *http.Response, error) {
	if path == "" {
...
func (m *apiClient) UpdateCredential(org, name, credentialType string, rawCred *models.CredentialRaw, path, canonical, description string) (*models.Credential, *http.Response, error) {
	if path == "" {
		re := regexp.MustCompile(`[^a-zA-z0-9_\-./]`)
		path = re.ReplaceAllString(name, "-")
	}
...
func (m *apiClient) GetCredential(org, credential string) (*models.Credential, *http.Response, error) {
	var result *models.Credential
	resp, err := m.GenericRequest(Request{
		Method:       "GET",
		Organization: &org,
...
func (m *apiClient) DeleteCredential(org, credential string) (*http.Response, error) {
	resp, err := m.GenericRequest(Request{
		Method:       "DELETE",
		Organization: &org,
		Route:        []string{"organizations", org, "credentials", credential},
...
func (m *apiClient) ListCredentials(org, credentialType string, filters ...LHSFilter) ([]*models.CredentialSimple, *http.Response, error) {
