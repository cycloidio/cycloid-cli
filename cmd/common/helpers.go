...
var (
	orgRe  = regexp.MustCompile(`\(\$ organization_canonical \$\)`)
...
func UpdateMapField(field, value string, m map[string]map[string]map[string]any) error {
	...
	if strings.HasPrefix(trimmedValue, "[") && strings.HasSuffix(trimmedValue, "]") || strings.HasPrefix(trimmedValue, "{") && strings.HasSuffix(trimmedValue, "}") {
		...
		if err != nil {
			return fmt.Errorf("invalid JSON value in key=val update with value '%s': %w", trimmedValue, err)
		}
