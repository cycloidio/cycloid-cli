...
func TestChildOrgCrud(t *testing.T) {
	...
	defer func() {
		_, err := m.DeleteOrganization(childOrg)
		if err != nil {
	...
	_, _, err := m.CreateOrganizationChild(config.Org, childOrg, nil)
	if err != nil {
	...
	newOrg, _, err := m.UpdateOrganization(childOrg, newName)
	if err != nil {
	...
	_, _, err = m.GetOrganization(childOrg)
	if err != nil {
