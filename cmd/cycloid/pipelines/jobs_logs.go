...
func streamJobLogs(cmd *cobra.Command, args []string) error {
	...
	if err != nil {
		return errors.Wrap(err, "unable to get --poll-interval flag")
	}
	if pollInterval <= 0 {
...
func latestBuildID(m middleware.Middleware, org, project, env, component, pipeline, job string) (string, error) {
