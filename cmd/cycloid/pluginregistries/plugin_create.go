...
func NewPluginCreateCommand() *cobra.Command {
	var cmd = &cobra.Command{
		...
		Example: `
	# Create a plugin in registry 1
	cy --org my-org plugin-registry plugin create --registry-id 1 --name my-plugin
`,
		RunE: createPlugin,
	}
...
func createPlugin(cmd *cobra.Command, args []string) error {
	...
	if err != nil {
		return printer.SmartPrint(p, nil, err, "unable to create registry plugin", printer.Options{}, cmd.OutOrStderr())
	}
