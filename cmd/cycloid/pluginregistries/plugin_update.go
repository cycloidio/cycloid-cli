...
func NewPluginUpdateCommand() *cobra.Command {
	var cmd = &cobra.Command{
		...
		Example: `
	# Update plugin 2 in registry 1
	cy --org my-org plugin-registry plugin update --registry-id 1 --plugin-id 2 --name new-name
`,
		RunE: updatePlugin,
	}
	...
	cmd.Flags().String("name", "", "new name for the plugin")
...
func updatePlugin(cmd *cobra.Command, args []string) error {
	...
	if err != nil {
		return printer.SmartPrint(p, nil, err, "unable to update registry plugin", printer.Options{}, cmd.OutOrStderr())
	}
