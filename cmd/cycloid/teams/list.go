...
func NewListTeamCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:               "list [team_canonicals...]",
		Short:             "List one or more teams information",
		RunE:              listTeam,
...
func listTeam(cmd *cobra.Command, args []string) error {
	...
	if err != nil {
		return printer.SmartPrint(p, nil, err, fmt.Sprintf("failed to list team: %w", err), printer.Options{}, cmd.OutOrStderr())
	}
