...
func deleteEnvironment(cmd *cobra.Command, args []string) error {
	if cyargs.IsSet(cmd, "project") {
		return fmt.Errorf("--project is no longer accepted on environment delete, did you mean cy environment unlink --project %s --env <env>", cmd.Flag("project").Value.String())
	}
