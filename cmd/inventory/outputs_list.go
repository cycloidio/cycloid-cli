...
`,
	...
	cyout.RegisterModel(cmd, apiclient.InventoryOutput{})
...
func outputsList(cmd *cobra.Command, args []string) error {
	...
	if key, _ := cmd.Flags().GetString("key"); key != "" {
		filters = append(filters, apiclient.LHSFilter{Attribute: "output_key", Condition: "eq", Value: key})
	}
	if outputType, _ := cmd.Flags().GetString("type"); outputType != "" {
		filters = append(filters, apiclient.LHSFilter{Attribute: "output_type", Condition: "eq", Value: outputType})
	}
	if cmd.Flags().Changed("pinned") {
		...
		filters = append(filters, apiclient.LHSFilter{Attribute: "output_is_pinned", Condition: "eq", Value: val})
	}
