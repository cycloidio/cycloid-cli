...
func createBuild(cmd *cobra.Command, args []string) error {
	...
	if err != nil {
		return fmt.Errorf("unable to get timeout flag: %w", err)
	}
	cancelOnTimeout, err := cmd.Flags().GetBool("watch-cancel-on-timeout")
	if err != nil {
		return fmt.Errorf("unable to get watch-cancel-on-timeout flag: %w", err)
	}
	...
	if !watch && timeoutSeconds > 0 {
		return fmt.Errorf("--timeout can only be used with --watch")
	}
	if !watch && cancelOnTimeout {
		return fmt.Errorf("--watch-cancel-on-timeout can only be used with --watch")
	}
	if watch && cancelOnTimeout && timeoutSeconds == 0 {
		return fmt.Errorf("--watch-cancel-on-timeout requires --timeout to be set")
	}
	...
	if watch {
		if build == nil || build.ID == nil {
			return fmt.Errorf("triggered build payload is missing id")
		}
