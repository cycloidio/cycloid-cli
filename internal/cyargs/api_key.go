...
func AddAPIKeyCanonicalFlag(cmd *cobra.Command) string {
	...
	_ = cmd.RegisterFlagCompletionFunc(flagName, CompleteAPIKeyCanonical)
	return flagName
}

func CompleteAPIKeyCanonical(cmd *cobra.Command, args []string, toComplete string) ([]cobra.Completion, cobra.ShellCompDirective) {
	org, err := GetOrg(cmd)
	if err != nil {
