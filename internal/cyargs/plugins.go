...
func CompletePluginInstallID(cmd *cobra.Command, args []string, toComplete string) ([]cobra.Completion, cobra.ShellCompDirective) {
	...
	m := apiclient.NewMiddleware(common.NewAPI())
...
func ResolvePluginInstallID(org, nameOrID string, m apiclient.Middleware) (uint32, error) {
...
func ResolvePluginManagerID(org, nameOrID string, m apiclient.Middleware) (uint32, error) {
	if id, err := strconv.ParseUint(nameOrID, 10, 32); err == nil {
		return uint32(id), nil
	}
	managers, _, err := m.ListPluginManagers(org)
...
func ResolvePluginRegistryID(org, nameOrID string, m apiclient.Middleware) (uint32, error) {
	if id, err := strconv.ParseUint(nameOrID, 10, 32); err == nil {
		return uint32(id), nil
	}
	registries, _, err := m.ListPluginRegistries(org)
...
func ResolveRegistryPluginID(org string, registryID uint32, nameOrID string, m apiclient.Middleware) (uint32, error) {
	if id, err := strconv.ParseUint(nameOrID, 10, 32); err == nil {
		return uint32(id), nil
	}
	plugins, _, err := m.ListRegistryPlugins(org, registryID)
