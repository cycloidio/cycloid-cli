...
func CompleteTeam(cmd *cobra.Command, args []string, toComplete string) ([]cobra.Completion, cobra.ShellCompDirective) {
	...
	created_at, _ := GetTeamCreatedAt(cmd)
	orderBy, _ := GetTeamOrderBy(cmd)

	api := common.NewAPI()
	m := middleware.NewMiddleware(api)

	teams, err := m.ListTeams(org, &name, created_at, &memberID, orderBy)
	if err != nil {
...
func AddTeamOwnerFlag(cmd *cobra.Command) string {
	cmd.Flags().StringP(teamOwnerFlagName, "o", "", "specify a team owner canonical")
