package test_utils

import (
	"context"
	"fmt"
	"strings"

	"github.com/testcontainers/testcontainers-go"
)

var (
	concourseAuthorizedWorkerKey = `ssh-rsa  ugo@dragonfly`
	concourseSessionSigningKey   = `-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
`
	concourseSessionSigningPublicKey = `ssh-rsa  ugo@dragonfly`
	concourseTsaHostKey              = `-----BEGIN RSA PRIVATE KEY-----

Xpwf1oIKDxZfiIxWtGLbQj1aMclohJ2qvzuNnx6YnASKVHw25EUg
-----END RSA PRIVATE KEY-----
`
	concourseTsaHostPublicKey = `ssh-rsa  ugo@dragonfly`
	concourseConfig           = `---
registry-image:
  registry_mirror:
    host: 'registry-mirror.owl.cycloid.io'

docker-image:
  registry_mirror: 'https://registry-mirror.owl.cycloid.io'
`
	concoursePort = "8000"
)

func GetConcourse(ctx context.Context, version,
	concourseUser, concoursePassword, postgresDatabase, postgresUser, postgresPassword, postgresHost, vaultHost, vaultRootToken, vaultPathPrefix string) (testcontainers.Container, error) {
	concourseEnv := map[string]string{
		"CONCOURSE_ADD_LOCAL_USER":              concourseUser + ":" + concoursePassword,
		"CONCOURSE_MAIN_TEAM_LOCAL_USER":        concourseUser,
		"CONCOURSE_BIND_PORT":                   concoursePort,
		"CONCOURSE_EXTERNAL_URL":                "http://0.0.0.0:" + concoursePort,
		"CONCOURSE_POSTGRES_HOST":               postgresHost,
		"CONCOURSE_POSTGRES_USER":               postgresUser,
		"CONCOURSE_POSTGRES_PASSWORD":           postgresPassword,
		"CONCOURSE_POSTGRES_DATABASE":           postgresDatabase,
		"CONCOURSE_BASE_RESOURCE_TYPE_DEFAULTS": "/brt/base_resource_types_defaults.yml",
		"CONCOURSE_VAULT_URL":                   vaultHost, // ,
		"CONCOURSE_VAULT_AUTH_BACKEND":          "token",
		"CONCOURSE_VAULT_CLIENT_TOKEN":          vaultRootToken,  // "root_token",
		"CONCOURSE_VAULT_PATH_PREFIX":           vaultPathPrefix, // "/cycloid",
		"CONCOURSE_VAULT_AUTH_BACKEND_MAX_TTL":  "3600s",
		"CONCOURSE_CLUSTER_NAME":                "dev",
	}
	concourseContainer, err := testcontainers.GenericContainer(ctx, testcontainers.GenericContainerRequest{
		ContainerRequest: testcontainers.ContainerRequest{
			Image:        "concourse/concourse:" + version,
			Env:          concourseEnv,
			ExposedPorts: []string{concoursePort},
			Networks:     []string{"cli-tests"},
			Tmpfs: map[string]string{
				"/database": "rw",
			},
			Files: []testcontainers.ContainerFile{
				{
					Reader:            strings.NewReader(concourseConfig),
					ContainerFilePath: "/brt/base_resource_types_defaults.yml",
					FileMode:          0444,
				},
				{
					Reader:            strings.NewReader(concourseAuthorizedWorkerKey),
					ContainerFilePath: "/concourse-keys/authorized_worker_keys",
					FileMode:          0444,
				},
				{
					Reader:            strings.NewReader(concourseSessionSigningKey),
					ContainerFilePath: "/concourse-keys/session_signing_key",
					FileMode:          0444,
				},
				{
					Reader:            strings.NewReader(concourseSessionSigningPublicKey),
					ContainerFilePath: "/concourse-keys/session_signing_key.pub",
					FileMode:          0444,
				},
				{
					Reader:            strings.NewReader(concourseTsaHostKey),
					ContainerFilePath: "/concourse-keys/tsa_host_key",
					FileMode:          0444,
				},
				{
					Reader:            strings.NewReader(concourseTsaHostPublicKey),
					ContainerFilePath: "/concourse-keys/tsa_host_key.pub",
					FileMode:          0444,
				},
			},
		},
		Started: true,
	})

	if err != nil {
		return nil, fmt.Errorf("fauled to create concourse container: %v", err)
	}

	return concourseContainer, nil
}
